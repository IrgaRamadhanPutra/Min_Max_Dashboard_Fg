    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const scrollableTbodies = document.querySelectorAll(
                '[id^="scrollable-tbody-"]'); // Pilih semua elemen dengan ID yang diawali 'scrollable-tbody-'

            let scrollSpeed = 60; // Sesuaikan kecepatan scroll (dalam milidetik)
            let scrollStep = 1; // Sesuaikan jumlah scroll per langkah

            // Fungsi untuk memulai scrolling
            function startScrolling() {
                scrollableTbodies.forEach(function(scrollableTbody) {
                    // Hentikan interval yang sebelumnya jika ada
                    if (scrollableTbody.dataset.interval) {
                        clearInterval(scrollableTbody.dataset.interval);
                    }

                    // Set interval untuk melakukan scroll
                    let intervalId = setInterval(() => {
                        // Tambahkan scrollStep ke posisi scroll saat ini
                        scrollableTbody.scrollTop += scrollStep;

                        // Periksa apakah sudah mencapai bagian bawah
                        if (scrollableTbody.scrollTop + scrollableTbody.clientHeight >=
                            scrollableTbody.scrollHeight) {
                            scrollableTbody.scrollTop = 0; // Atur kembali scroll ke posisi atas
                        }
                    }, scrollSpeed);

                    // Simpan ID interval di dataset untuk penghapusan di kemudian hari
                    scrollableTbody.dataset.interval = intervalId;
                });
            }

            // Mulai scrolling saat DOM siap
            startScrolling();

            // Tangani event resize jendela
            window.addEventListener('resize', function() {
                // Reset posisi scroll untuk setiap tbody
                scrollableTbodies.forEach(function(scrollableTbody) {
                    scrollableTbody.scrollTop = 0; // Kembali ke posisi atas
                });
                startScrolling(); // Mulai ulang scrolling dengan dimensi yang diperbarui
            });
        });
